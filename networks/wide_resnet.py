...
from networks.train_plot import PlotLearning  # Assuming this is custom

# Code adapted for TensorFlow 2.x
class WideResNet:
    ...
    def wide_residual_network(self, img_input, classes_num, depth, k):
        ...
        def residual_block(x, out_filters, increase_filter=False):
            ...
            conv_1 = Conv2D(out_filters, kernel_size=(3, 3), strides=first_stride, padding='same',
                            kernel_initializer=HeNormal(), kernel_regularizer=regularizers.l2(self.weight_decay))(pre_relu)
            bn_1 = BatchNormalization()(conv_1)
            relu1 = Activation('relu')(bn_1)
            conv_2 = Conv2D(out_filters, kernel_size=(3, 3), strides=(1, 1), padding='same',
                            kernel_initializer=HeNormal(), kernel_regularizer=regularizers.l2(self.weight_decay))(relu1)
            if increase_filter or in_filters != out_filters:
                projection = Conv2D(out_filters, kernel_size=(1, 1), strides=first_stride, padding='same',
                                    kernel_initializer=HeNormal(), kernel_regularizer=regularizers.l2(self.weight_decay))(x)
        ...
        x = Dense(classes_num, activation='softmax', kernel_initializer=HeNormal(),
                  kernel_regularizer=regularizers.l2(self.weight_decay))(x)
        return x
